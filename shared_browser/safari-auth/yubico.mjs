...
const approvedOrigins=['https://demo.yubico.com','https://cryptoagent-1-1.agent-trace.ts.net:3581'];
const origins=(process.env.AUTH_SITES||process.env.AUTH_SITE||approvedOrigins.join(',')).split(',');
if(!origins.length||origins.some(origin=>!approvedOrigins.includes(origin)))throw Error('Origin is not approved for this companion build');
const relayURL='http://localhost:8811';
...
await fs.writeFile(path.join(local,'extension/config.js'),'const AUTH_CONFIG = '+JSON.stringify({relay:relayURL,sites:origins,token,label:'procbox shared browser'})+';\n',{mode:0o600});
...
http.createServer(async(req,res)=>{try{
  ...
  if(host){
    if(req.url==='/host/start'){
      ...
      if(!origins.includes(origin))throw Error('Unsupported origin');
...
}catch(error){console.log('Relay rejected:',error.message);json(res,{error:error.message},400);}}).listen(8811,'127.0.0.1',()=>console.log('Auth companion ready for '+origins.join(', ')));
