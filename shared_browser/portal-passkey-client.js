...
button.onclick=async()=>{button.disabled=true;try{const response=await startAuthentication({optionsJSON:phoneOptions(request.publicKey)});await api('respond',{response});status.textContent='Passkey response sent. The dashboard will verify it before signing the agent in.';}catch(e){status.textContent=e.message;button.disabled=false;}};
